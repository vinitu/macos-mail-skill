-- Get full message details by message-id. argv: accountName mailboxName messageId
-- Output: one JSON object.
on run argv
	if (count of argv) < 3 then error "Usage: get-by-id.applescript <account> <mailbox> <message-id>"
	...
	tell application "Mail"
		with timeout of 30 seconds
			...
			set matchingMessages to (messages of targetMailbox whose message id is targetId)
			if (count of matchingMessages) is 0 then error "Message not found: " & targetId
			set msgRecord to item 1 of matchingMessages
			return my messageJson(msgRecord, accName, mbName)
		end timeout
	end tell
end run

on messageJson(msgRecord, accName, mbName)
	set identityValue to my messageIdentity(msgRecord, accName, mbName)
	...
	return "{" & "\"id\":" & my jsonString(identityValue) & "," & "\"account\":" & my jsonString(accName) & "," & "\"mailbox\":" & my jsonString(mbName) & "," & "\"index\":null," & "\"subject\":" & my jsonString(subjectValue) & "," & "\"sender\":" & my jsonString(senderValue) & "," & "\"date_received\":" & dateReceivedValue & "," & "\"date_sent\":" & dateSentValue & "," & "\"message_id\":" & my jsonNullable(messageIdValue) & "," & "\"reply_to\":" & replyToValue & "," & "\"message_size\":" & messageSizeValue & "," & "\"read\":" & readValue & "," & "\"flagged\":" & flaggedValue & "," & "\"junk\":" & junkValue & "," & "\"flag_index\":" & flagIndexValue & "," & "\"background_color\":" & backgroundColorValue & "," & "\"all_headers\":" & allHeadersValue & "," & "\"content\":" & my jsonString(contentValue) & "}"
end messageJson

on messageIdentity(msgRecord, accName, mbName)
	using terms from application "Mail"
		set rawMessageIdValue to (message id of msgRecord)
	end using terms from
	set messageIdValue to my safeText(rawMessageIdValue)
	if messageIdValue is "" then
		return accName & "/" & mbName & "/message-id"
	end if
