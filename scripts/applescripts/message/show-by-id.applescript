-- Show a message in Mail.app UI by message-id. argv: accountName mailboxName messageId
-- Output: one JSON object.
on run argv
	if (count of argv) < 3 then error "Usage: show-by-id.applescript <account> <mailbox> <message-id>"
	...
	tell application "Mail"
		with timeout of 30 seconds
			...
			set identityValue to my messageIdentity(targetMessage, accName, mbName)
			...
			return "{" & "\"shown\":true," & "\"id\":" & my jsonString(identityValue) & "," & "\"account\":" & my jsonString(accName) & "," & "\"mailbox\":" & my jsonString(mbName) & "," & "\"index\":null," & "\"subject\":" & my jsonString(subjectValue) & "}"
		end timeout
	end tell
end run
