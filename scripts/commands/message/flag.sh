...
usage() {
  echo "Usage: scripts/commands/message/flag.sh <account-name> <mailbox-name> <index>" >&2
}
...
main() {
  ...
  local flag_script
  flag_script=$(require_backend_script "message" "flag") || exit 1

  capture_osascript "$flag_script" "$account_name" "$mailbox_name" "$index" >/dev/null
