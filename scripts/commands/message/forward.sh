...
usage() {
  echo "Usage: scripts/commands/message/forward.sh <account-name> <mailbox-name> <index>" >&2
}
...
main() {
  ...
  local forward_script
  forward_script=$(require_backend_script "message" "forward") || exit 1

  capture_osascript "$forward_script" "$account_name" "$mailbox_name" "$index" >/dev/null
