...
usage() {
  echo "Usage: scripts/commands/message/mark-unread.sh <account-name> <mailbox-name> <index>" >&2
}
...
main() {
  ...
  local mark_unread_script
  mark_unread_script=$(require_backend_script "message" "mark-unread") || exit 1

  capture_osascript "$mark_unread_script" "$account_name" "$mailbox_name" "$index" >/dev/null
