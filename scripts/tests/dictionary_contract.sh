...
tmp_standard="$(mktemp)"
cleanup() {
  rm -f "$tmp_mail" "$tmp_standard"
}
trap cleanup EXIT

make --no-print-directory dictionary-mail >"$tmp_mail"
...
has_pattern '<class name="account"' "$tmp_mail"
has_pattern '<class name="message"' "$tmp_mail"
has_pattern '<class name="mailbox"' "$tmp_mail"
